...
app.defaultConfiguration = function defaultConfiguration() {
  ...
  // 这个是为了给./request.js req.subdomain使用的 
...
app.lazyrouter = function lazyrouter() {
  if (!this._router) {
    this._router = new Router({
      ...
      // strict 设置为严格模式，什么是严格模式？
      // 主要指末尾的斜杠，如果路由中间件app.get('/user')，那么请求get('/user/')是匹配不上的 
...
app.use = function use(fn) {
  ...
  // arg 如果是一个对象，则没有 slice 方法，所有需要调用 slice.call()将 arg 转成数组
  ...
  // setup router
  // 主要是给app添加_router#Router这个属性，同时添加中间件express.Init和qs两个中间件
  this.lazyrouter();
  var router = this._router;
